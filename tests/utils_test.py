...
from blocknative.utils import raise_error_on_status, is_server_echo, network_id_to_name_ethereum, network_id_to_name_bitcoin
...
class TestNetworkIdNameTableEthereum(unittest.TestCase):
    ID_LIST = [1, 3, 4, 5, 42, 56, 100]
    EXP_LIST = ['main', 'ropsten', 'rinkeby', 'goerli', 'kovan', 'bsc-main', 'xdai']

    def test_network_id_to_name(self):
        for i, nid in enumerate(self.ID_LIST):
            self.assertEqual(self.EXP_LIST[i], network_id_to_name_ethereum(nid))

class TestNetworkIdNameTableBitcoin(unittest.TestCase):
    ID_LIST = [1, 2]
    EXP_LIST = ['main', 'testnet']

    def test_network_id_to_name(self):
        for i, nid in enumerate(self.ID_LIST):
            self.assertEqual(self.EXP_LIST[i], network_id_to_name_bitcoin(nid))
